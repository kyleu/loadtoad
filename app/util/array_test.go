...
func TestArrayToStringArray(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Convert []interface{} to []any for the test
			anySlice := make([]any, len(tt.input))
...
func TestStringArrayFromAny(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "mixed types no truncation",
			input:     []any{1, "hello", true, []byte("world")},
			maxLength: 0,
			expected:  []string{"1", "hello", "true", "world"},
		},
		{
			name:      "with truncation",
			input:     []any{"abcdefghij", "12345"},
			maxLength: 5,
			expected:  []string{"abcde... (truncated)", "12345"},
		},
	}
...
func TestArrayTransform(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "empty array",
			input:    []int{},
			fn:       func(i int) string { return "x" + string(rune(i+'0')) },
			expected: []string{},
		},
		{
...
func TestArrayRemoveNil(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*int{&i1, &i2, &i3},
			expected: []*int{&i1, &i2, &i3},
		},
		{
			name:     "some nil elements",
			input:    []*int{&i1, nil, &i2, nil, &i3},
			expected: []*int{&i1, &i2, &i3},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check content - can't use DeepEqual directly because we're comparing pointers
...
func TestArrayRemoveEmpty(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "some empty elements",
			input:    []string{"a", "", "b", "", "c"},
			expected: []string{"a", "b", "c"},
		},
		{
			name:     "all empty elements",
			input:    []string{"", "", ""},
			expected: []string{},
		},
	}
	...
	intTests := []struct {
		...
		{
			name:     "no zero elements",
			input:    []int{1, 2, 3},
			expected: []int{1, 2, 3},
		},
		{
			name:     "some zero elements",
			input:    []int{1, 0, 2, 0, 3},
			expected: []int{1, 2, 3},
		},
		{
...
func TestArrayFirstN(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "n larger than array",
			input:    []int{1, 2, 3},
			n:        5,
			expected: []int{1, 2, 3},
		},
		{
			name:     "n smaller than array",
			input:    []int{1, 2, 3, 4, 5},
			n:        3,
			expected: []int{1, 2, 3},
		},
		{
			name:     "n equal to array length",
			input:    []int{1, 2, 3},
			n:        3,
			expected: []int{1, 2, 3},
		},
...
func TestArrayReplaceOrAdd(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			fn:       func(i int) bool { return i == 2 },
			replace:  42,
			expected: []int{1, 3, 5, 42},
		},
		{
			name:     "multiple matches (should replace first)",
			input:    []int{1, 2, 3, 2, 4},
			fn:       func(i int) bool { return i == 2 },
			replace:  42,
			expected: []int{1, 42, 3, 2, 4},
		},
	}
...
func TestMapError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:        "empty array",
			input:       []int{},
			fn:          func(i, idx int) (string, error) { return fmt.Sprintf("x%d", i), nil },
			expected:    []string{},
			expectError: false,
		},
		{
		...
		},
		{
			name:  "with error",
			input: []int{1, 2, 3, 4, 5},
			fn: func(i, idx int) (string, error) {
				if i == 3 {
					return "", fmt.Errorf("error at value %d", i)
				}
