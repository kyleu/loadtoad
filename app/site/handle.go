...
func Handle(path []string, as *app.State, ps *cutil.PageState) (string, layout.Page, []string, error) {
	...
	case util.AppKey:
		msg := "\n  " +
			"<meta name=\"go-import\" content=\"github.com/kyleu/loadtoad git %s\">\n  " +
			"<meta name=\"go-source\" content=\"github.com/kyleu/loadtoad %s %s/tree/master{/dir} %s/blob/master{/dir}/{file}#L{line}\">"
	...
	case keyDownload:
		...
		ps.Title = "Downloads"
		ps.Data = util.ValueMap{"base": "https://github.com/kyleu/loadtoad/releases/download/v" + as.BuildInfo.Version, "links": dls}
