// Code generated by qtc from "Detail.html". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line views/vfile/Detail.html:1
package vfile

//line views/vfile/Detail.html:1
import (
	"encoding/base64"
	"strings"
	"unicode/utf8"

	"github.com/kyleu/loadtoad/app"
	"github.com/kyleu/loadtoad/app/controller/cutil"
	"github.com/kyleu/loadtoad/app/lib/filesystem"
	"github.com/kyleu/loadtoad/app/util"
	"github.com/kyleu/loadtoad/views/components"
)

//line views/vfile/Detail.html:13
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line views/vfile/Detail.html:13
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line views/vfile/Detail.html:13
func StreamDetail(qw422016 *qt422016.Writer, as *app.State, path []string, size int64, b []byte, urlPrefix string, additionalLinks map[string]string, ps *cutil.PageState, lineNumLinkAndTitle ...string) {
//line views/vfile/Detail.html:13
	qw422016.N().S(`
`)
//line views/vfile/Detail.html:15
	fileSize := size
	if fileSize == 0 {
		fileSize = int64(len(b))
	}

//line views/vfile/Detail.html:20
	if additionalLinks != nil && len(additionalLinks) > 0 {
//line views/vfile/Detail.html:20
		qw422016.N().S(`  <div class="right">
`)
//line views/vfile/Detail.html:22
		for k, v := range additionalLinks {
//line views/vfile/Detail.html:24
			var icon string
			if iconIdx := strings.Index(k, "**"); iconIdx > -1 {
				icon = k[iconIdx+2:]
				k = k[:iconIdx]
			}
			var newWindow bool
			if strings.HasPrefix(k, "!") {
				newWindow = true
				k = k[1:]
			}

//line views/vfile/Detail.html:34
			qw422016.N().S(`    <a href="`)
//line views/vfile/Detail.html:35
			qw422016.E().S(v)
//line views/vfile/Detail.html:35
			qw422016.N().S(`"`)
//line views/vfile/Detail.html:35
			if newWindow {
//line views/vfile/Detail.html:35
				qw422016.N().S(` target="_blank"`)
//line views/vfile/Detail.html:35
			}
//line views/vfile/Detail.html:35
			qw422016.N().S(`><button>`)
//line views/vfile/Detail.html:35
			if icon != "" {
//line views/vfile/Detail.html:35
				components.StreamSVGButton(qw422016, icon, ps)
//line views/vfile/Detail.html:35
				qw422016.N().S(` `)
//line views/vfile/Detail.html:35
			}
//line views/vfile/Detail.html:35
			qw422016.E().S(k)
//line views/vfile/Detail.html:35
			qw422016.N().S(`</button></a>
`)
//line views/vfile/Detail.html:36
		}
//line views/vfile/Detail.html:36
		qw422016.N().S(`  </div>
`)
//line views/vfile/Detail.html:38
	}
//line views/vfile/Detail.html:38
	qw422016.N().S(`  <h3>
`)
//line views/vfile/Detail.html:40
	for idx, p := range path {
//line views/vfile/Detail.html:40
		qw422016.N().S(`/<a href="`)
//line views/vfile/Detail.html:40
		qw422016.E().S(urlPrefix)
//line views/vfile/Detail.html:40
		qw422016.N().S(`/`)
//line views/vfile/Detail.html:40
		qw422016.E().S(util.StringFilePath(path[:idx+1]...))
//line views/vfile/Detail.html:40
		qw422016.N().S(`">`)
//line views/vfile/Detail.html:40
		qw422016.E().S(p)
//line views/vfile/Detail.html:40
		qw422016.N().S(`</a>`)
//line views/vfile/Detail.html:40
	}
//line views/vfile/Detail.html:40
	qw422016.N().S(`    <em>(`)
//line views/vfile/Detail.html:41
	qw422016.E().S(util.ByteSizeSI(fileSize))
//line views/vfile/Detail.html:41
	qw422016.N().S(`)</em>
  </h3>
  <div class="mt">
`)
//line views/vfile/Detail.html:44
	if fileSize > (1024 * 128) {
//line views/vfile/Detail.html:44
		qw422016.N().S(`    <em>File is `)
//line views/vfile/Detail.html:45
		qw422016.E().S(util.ByteSizeSI(fileSize))
//line views/vfile/Detail.html:45
		qw422016.N().S(` bytes, which is too large for the file viewer</em>
`)
//line views/vfile/Detail.html:46
	} else if utf8.Valid(b) {
//line views/vfile/Detail.html:47
		out, _ := cutil.FormatFilename(string(b), path[len(path)-1], lineNumLinkAndTitle...)

//line views/vfile/Detail.html:47
		qw422016.N().S(`    `)
//line views/vfile/Detail.html:48
		qw422016.N().S(out)
//line views/vfile/Detail.html:48
		qw422016.N().S(`
`)
//line views/vfile/Detail.html:49
	} else {
//line views/vfile/Detail.html:49
		qw422016.N().S(`
`)
//line views/vfile/Detail.html:51
		if imgType := filesystem.ImageType(path...); imgType != "" {
//line views/vfile/Detail.html:51
			qw422016.N().S(`    <img alt="Image of type [`)
//line views/vfile/Detail.html:52
			qw422016.E().S(imgType)
//line views/vfile/Detail.html:52
			qw422016.N().S(`]" src="data:image/`)
//line views/vfile/Detail.html:52
			qw422016.E().S(imgType)
//line views/vfile/Detail.html:52
			qw422016.N().S(`;base64,`)
//line views/vfile/Detail.html:52
			qw422016.E().S(base64.StdEncoding.EncodeToString(b))
//line views/vfile/Detail.html:52
			qw422016.N().S(`" />
    <hr />
`)
//line views/vfile/Detail.html:54
		}
//line views/vfile/Detail.html:54
		qw422016.N().S(`
`)
//line views/vfile/Detail.html:56
		exif, err := filesystem.ExifExtract(b)

//line views/vfile/Detail.html:57
		if err == nil {
//line views/vfile/Detail.html:57
			qw422016.N().S(`    <div class="overflow full-width">
      <table>
        <thead>
          <tr>
            <th>EXIF Name</th>
            <th>Value</th>
          </tr>
        </thead>
        <tbody>
`)
//line views/vfile/Detail.html:67
			for k, v := range exif {
//line views/vfile/Detail.html:67
				qw422016.N().S(`          <tr>
            <td>`)
//line views/vfile/Detail.html:69
				qw422016.E().S(k)
//line views/vfile/Detail.html:69
				qw422016.N().S(`</td>
            <td>`)
//line views/vfile/Detail.html:70
				qw422016.E().V(v)
//line views/vfile/Detail.html:70
				qw422016.N().S(`</td>
          </tr>
`)
//line views/vfile/Detail.html:72
			}
//line views/vfile/Detail.html:72
			qw422016.N().S(`        </tbody>
      </table>
    </div>
`)
//line views/vfile/Detail.html:76
		} else {
//line views/vfile/Detail.html:76
			qw422016.N().S(`    <em>File is binary and contains no exif header</em>
`)
//line views/vfile/Detail.html:78
		}
//line views/vfile/Detail.html:79
	}
//line views/vfile/Detail.html:79
	qw422016.N().S(`  </div>
`)
//line views/vfile/Detail.html:81
}

//line views/vfile/Detail.html:81
func WriteDetail(qq422016 qtio422016.Writer, as *app.State, path []string, size int64, b []byte, urlPrefix string, additionalLinks map[string]string, ps *cutil.PageState, lineNumLinkAndTitle ...string) {
//line views/vfile/Detail.html:81
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/vfile/Detail.html:81
	StreamDetail(qw422016, as, path, size, b, urlPrefix, additionalLinks, ps, lineNumLinkAndTitle...)
//line views/vfile/Detail.html:81
	qt422016.ReleaseWriter(qw422016)
//line views/vfile/Detail.html:81
}

//line views/vfile/Detail.html:81
func Detail(as *app.State, path []string, size int64, b []byte, urlPrefix string, additionalLinks map[string]string, ps *cutil.PageState, lineNumLinkAndTitle ...string) string {
//line views/vfile/Detail.html:81
	qb422016 := qt422016.AcquireByteBuffer()
//line views/vfile/Detail.html:81
	WriteDetail(qb422016, as, path, size, b, urlPrefix, additionalLinks, ps, lineNumLinkAndTitle...)
//line views/vfile/Detail.html:81
	qs422016 := string(qb422016.B)
//line views/vfile/Detail.html:81
	qt422016.ReleaseByteBuffer(qb422016)
//line views/vfile/Detail.html:81
	return qs422016
//line views/vfile/Detail.html:81
}
